  ...
  return (tool as any)(
    ...
    },
    {
      ...
      schema: z.object({
        ...
        continueOnError: z.boolean().optional().describe('Continue sending remaining invoices if one fails (default: true)'),
      }),
    }
  );
}
...
const FA3ItemSchema = z.object({
  ...
  unit: z.string().optional().describe('Unit (e.g. szt., godz., kg); defaults to szt.'),
  ...
  return (tool as any)(
    ...
    },
    {
      ...
      schema: z.object({
        ...
        sellerNip: z.string().optional().describe('Seller NIP (10 digits, no dashes or spaces). Will be looked up from contractor DB if not provided.'),
        sellerStreet: z.string().optional().describe('Seller street address (e.g. ul. Marszałkowska 1). Will be looked up from contractor DB if not provided.'),
        sellerCity: z.string().optional().describe('Seller city. Will be looked up from contractor DB if not provided.'),
        sellerZip: z.string().optional().describe('Seller postal code (e.g. 00-001). Will be looked up from contractor DB if not provided.'),
        sellerCountry: z.string().optional().describe('Seller country code (default: PL)'),
        buyerName: z.string().describe('Buyer company or person name'),
        buyerNip: z.string().optional().describe('Buyer NIP number. Will be looked up from contractor DB if not provided.'),
        buyerStreet: z.string().optional().describe('Buyer street address. Will be looked up from contractor DB if not provided.'),
        buyerCity: z.string().optional().describe('Buyer city. Will be looked up from contractor DB if not provided.'),
        buyerZip: z.string().optional().describe('Buyer postal code. Will be looked up from contractor DB if not provided.'),
        buyerCountry: z.string().optional().describe('Buyer country code (default: PL)'),
        ...
        paymentMethod: z.string().optional().describe('Payment method (default: transfer)'),
        paymentAccount: z.string().optional().describe('Bank account number (IBAN, optional)'),
      }),
    }
  );
}
